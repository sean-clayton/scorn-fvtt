...
import { ScornActorSheet } from "./actor/actor-sheet.js";
import { ScornItem } from "./item/item.js";
import { ScornItemSheet } from "./item/item-sheet.js";
...
import { ScornCombat } from "./combat.js";
import { createScornMacro, rollItemMacro } from "./macros.js";
...
Hooks.once("init", async function () {
  ...
  CONFIG.Actor.documentClass = ScornActor;
  CONFIG.Item.documentClass = ScornItem;

  // configure combat
  CONFIG.Combat.documentClass = ScornCombat;
  ...
  Actors.registerSheet("scorn", ScornActorSheet, { makeDefault: true });
  Items.unregisterSheet("core", ItemSheet);
  Items.registerSheet("scorn", ScornItemSheet, { makeDefault: true });

  configureHandleBar();
});

Hooks.once("ready", () => {
  Hooks.on("hotbarDrop", (bar, data, slot) => createScornMacro(data, slot));
});
...
Hooks.on("renderChatMessage", (message, html, data) => {
  ...
  if (actor !== undefined) {
    if (actor.testUserPermission(game.user, "OWNER") || game.user.isGM) {
      html.find(".roll-str-save").click(() => Damage._rollStrSave(actor, html));
    } else {
...
const configureHandleBar = () => {
  // Pre-load templates
  const templatePaths = ["systems/scorn/templates/parts/items-list.html"];
