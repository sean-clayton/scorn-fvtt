...
export class Damage {
  ...
  static applyToTarget(target, damage) {
    ...
    // Linked to Actor
    if (tokenDoc.isLinked || tokenDoc.isEmbedded) {
      ...
      const { dmg, newHp, newStr } = this._calculateHpAndStr(
        damage,
        armor,
        hp,
        str
    ...
    // Synthetic actor
      ...
      tokenDoc.modifyActorDocument({
        "system.hp.value": newHp,
        "system.abilities.STR.value": newStr,
      });

      const actor =
        tokenDoc.actorData !== undefined ? tokenDoc.actorData : tokenDoc.actor;
   ...
   * @param {*} data
   */
  static onClickChatMessageApplyButton(event, html) {
    ...
    if (event.shiftKey) {
      for (let index = 0; index < targetsList.length; index++) {
        ...
        const releaseOthers =
          index == 0 ? (!token.isTargeted ? true : false) : false;
  ...
  static _showDetails(data) {
    ...
    let content =
      "<p><strong>" +
      game.i18n.localize("SCORN.Damage") +
      "</strong>: " +
      dmg +
      " (" +
      damage +
    ...
    if (newHp !== hp) {
      content +=
        "<p><strong>" +
        game.i18n.localize("SCORN.HitProtection") +
        "</strong>: <s>" +
        hp +
        "</s> => " +
        newHp +
    ...
    if (newStr < str) {
      if (newStr === 0) {
        content += "<strong>" + game.i18n.localize("SCORN.Dead") + "</strong>";
      } else {
        content +=
          "<p><strong>" + game.i18n.localize("SCORN.StrSave") + "</strong></p>";
        content +=
          '<button type="button" class="roll-str-save">' +
          game.i18n.localize("SCORN.RollStrSave") +
          "</button>";
      }
    } else if (newHp === 0 && hp !== 0) {
      content +=
        "<p><strong>" + game.i18n.localize("SCORN.Scars") + "</strong></p>";
  ...
  static async _rollScarsTable(damage) {
    const table = await findCompendiumItem("scorn.utils", "Scars");
  ...
  static async _rollStrSave(actor, html) {
    ...
    const label = game.i18n.format("SCORN.Save", {
      key: game.i18n.localize("STR"),
    });
